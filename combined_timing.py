...
from huffman_baitidega import huff_encode, huff_decode
...
def plotResults(results, compressors, BWTs, times, save=False):
    ...
    plt.grid()
    if save:
        plt.savefig(str(nr) + "figure.png")
    else:
        plt.show()


def runSingleText(text, mult=1):
    runWithBWT(BWTSuffixesIndexes, encoder=huff_encode, decoder=huff_decode, text=text*mult)
    runWithBWT(BWTSuffixesIndexes, encoder=huff_encode, decoder=huff_decode, text=text*mult, include_BWT_time=False)
    runWithoutBWT(encoder=huff_encode, decoder=huff_decode, text=text*mult)
    print("================ LZ77 ===============")
...
#plotResults(results, compressors=["Huffman", "LZ77"], BWTs=["without BWT", "with BWT -", "with BWT"], times=[0, 1, 2], save=True)
nr += 1
plotResults(results, compressors=["Huffman"], BWTs=["without BWT", "with BWT -", "with BWT"], times=[2], save=True)
nr += 1
plotResults(results, compressors=["LZ77"], BWTs=["without BWT", "with BWT -", "with BWT"], times=[2], save=True)
